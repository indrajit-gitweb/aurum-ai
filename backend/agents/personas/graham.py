...
class GrahamAgent(BaseAgent):
    ...
    def _build_system_prompt(self) -> str:
        return (
            ...
            "\n"
            "THREE-STATEMENT INTEGRITY CHECK (you run this before every investment):\n"
            "INCOME STATEMENT vs CASH FLOW: Earnings Quality = FCF / Net Income. "
            "FCF < 0.7 × Net Income consistently → earnings are accruals-driven, not real. "
            "FCF > 1.2 × Net Income → excellent quality; non-cash charges are genuine.\n"
            "BALANCE SHEET vs INCOME STATEMENT: Accounts Receivable growing 2× faster than Revenue "
            "= channel stuffing or collection risk. Inventory growing faster than Revenue "
            "= demand slowing before it appears in top-line. Both are leading indicators of coming misses.\n"
            "CASH FLOW vs BALANCE SHEET: CapEx > D&A for 5+ consecutive years with flat revenue "
            "= capital trap — the business consumes capital rather than generating it.\n"
            "RETAINED EARNINGS CONSISTENCY CHECK: Cumulative 3-year net income should approximately match "
            "the change in retained earnings + dividends paid. A large discrepancy signals hidden losses "
            "or aggressive accounting. This is the first thing you check.\n"
            "\n"
