...
class NewsSentimentAgent(BaseAgent):
    ...
    def _build_system_prompt(self) -> str:
        return (
            ...
            "   HARD catalysts (high certainty, time-bound): Earnings date, FDA decision, "
            "   contract announcement, regulatory ruling, investor day, index rebalance.\n"
            "   SOFT catalysts (speculative): Rumoured M&A, analyst upgrade cycle, sector rotation.\n"
            "   EARNINGS CATALYST WEIGHTING: Next earnings date within 30 days = amplify all signals. "
            "   A bullish news trend + earnings in <30 days = elevated conviction. "
            "   Always flag: is the current news flow PRE-earnings positioning or POST-earnings reaction?\n"
            "\n"
            "5. SENTIMENT REGIME:\n"
            "   CONTRARIAN OPPORTUNITY: Overwhelmingly negative news (score < -0.3) + insider net buying "
            "   > $500k = potential bottom. Score this regime 70+ confidence.\n"
            "   MOMENTUM PLAY: Consistently positive news (score > +0.3) + accelerating coverage volume "
            "   + no insider selling = ride the wave.\n"
            "   DISTRIBUTION: Positive news headlines but insiders selling > $1M net = smart money exit. "
            "   Flag as HIGH RISK regardless of headline sentiment score.\n"
            "   NOISE: Mixed signals with low volume news + small/options insider transactions = "
            "   insufficient signal, confidence < 40.\n"
            "\n"
