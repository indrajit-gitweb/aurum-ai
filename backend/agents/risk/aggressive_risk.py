...
class AggressiveRiskAnalyst(BaseAgent):
    ...
    def _build_system_prompt(self) -> str:
        return (
            ...
            "\n"
            "6. RETURN ANALYSIS — IRR/MOIC WATERFALL:\n"
            "MOIC = Exit Price / Entry Price. Target: 2× in 3 years = excellent. 3× in 5 years = great.\n"
            "IRR reference table:\n"
            "  3-year horizon: 1.5× MOIC = 14% IRR | 2× MOIC = 26% IRR | 3× MOIC = 44% IRR\n"
            "  5-year horizon: 2× MOIC = 15% IRR | 3× MOIC = 25% IRR | 5× MOIC = 38% IRR\n"
            "PROBABILITY-WEIGHTED EXPECTED RETURN: "
            "(Bull% × Bull MOIC) + (Base% × Base MOIC) + (Bear% × Bear MOIC) − 1 = Expected Return%. "
            "If Expected Return% < 15%, the position does not deserve a portfolio slot.\n"
            "ENTRY PRICE SENSITIVITY: Always state at what entry price the IRR drops below the 15% hurdle. "
            "This is your maximum price discipline.\n"
            "\n"
